---=====================================
---custom dialog lib v1.11b by Muki
---自定义对话库 by Muki
---=====================================
...
---将lib.sentence的px、py、tx、ty由ui系坐标改为world系坐标偏移值
---修复shader文字效果无法指定范围的问题
---修复使用sentence_ex时气泡重叠导致无法使用透明对话框的问题
---修复float系气泡两行文字显示位置错误的问题

---@class custom_dialog @自定义对话库
custom_dialog = { list = {} }
local lib = custom_dialog
...
if not sp.string then sp.string = lib.spstring end --在无法调用sp.string时启用备用spstring库

----------------------------------------------------------------------------------------------------------------------------------------------------------------
---本自定义库附加系统介绍：文字效果
---文字效果可以在本对话库的所有对话函数中使用，只需在传入文本中按以下语法输入即可
---灵感来自AiC的文字效果，语法略有改动，格式为<flag param >text<end>（注意左边的>前面要空格）
---以下是详细语法规则：

---一、右边的<end>可以省略不写，此时文字效果范围持续至本句结尾

---[=[二、目前每句支持最多9个文字效果
---若要使用多个文字效果，请在flag与end后添加数字序号，文字效果将按序号顺序执行
---例：'<Col2 255,255,0,0><col1 0xFFFFFFFF >text<end1><end2>'将把text改为红色
---而'<Col1 255,255,0,0><col2 0xFFFFFFFF >text<end1><end2>'将把text改为白色
---多文字效果下<end>不可省略，且每个文字效果作用范围由对应<end>决定]=]

---二、多文字效果制作未完成，目前每句最多只支持一个文字效果

---三、若要在文字效果中使用变量，请使用连接符
---例：'<col '..self.co..' >'..text

---四、若传入的参数中有字符串，请直接写出，不需要再加引号
---例：'<snd tan00 >text<end>'

---五、若传入的参数多于一个，请将文字效果大写，并将参数放在{}中
---由于内部通过英文逗号数量来判断参数数量，参数内不能含有英文逗号
---例：'<Col {255,255,255,255} >text<end>'

---六、如需使用每帧自动变化的随机数，可以使用以下随机常量：
---ranA = ran:Float(0,360)
---ranB = ran:Float(0,1)
---ranC = ran:Sign()
---请注意，以上随机常量仅在当前文字效果接受number时可用，否则直接显示原字符
...
---<col color >text<end>
---将text的颜色设为color（传入十六进制数）

---<Col {alpha,red,green,blue} >text<end>
---将text的颜色设为color（传入argb分量）

---<fon font >text<end>
---修改显示text的字体为font（注：不会更改字号，请自行注意文字大小）

---<upt uppertext >text<end>
---在text上方以小字显示uppertext，颜色与字体和text相同

---<sca scale >text<end>
---将text的缩放比设为scale，并自动设定其位置以居中显示
...
---<img image >text<end>
---修改text显示时的对话图像为image

---<sha shake >text<end>
---使text显示时附带坐标最大偏移值为shake的颤抖效果（随机xy坐标变化）

---<pef shader_name >text<end>
---在渲染text时调用指定shader
...
---sp对话显示器
lib.sp_dialog_displayer = Class(object)
...
function lib.sp_dialog_displayer:init(p_dialog, dialogbox, dialogbox_hscale, dialogbox_vscale, dialogbox_yu, text_yu,
...
function lib.sp_dialog_displayer:frame()
    task.Do(self)
    if self.t > 0 then
        self.t = self.t - 1
    end
...
function lib.sp_dialog_displayer:render()
...
function lib.sp_dialog_displayer:del()
...
function lib.sp_sentence(self, img, pos, text, canskip, t, hscale, vscale, num, px, py, snd, vol, img_upper, rawtext,
                         fulltext)
    if self.dialog_displayer.type ~= 'sp' then
        lib.sp_SetDisplayer(self)
    end
    ...
    if not rawtext and flag then
        for k, v in ipairs(flag) do
            if v == 'snd' then snd = param[k] end
            if v == 'vol' then vol = lib.StrToNum(param[k]) end
            if v == 'img' then img = param[k] end
            if v == 'pef' and not lstg.tmpvar.TextEffect_RT_Created then
...
---@param img_upper boolean @对话图像图层是否高于对话框
function lib.sp_sentence_ex(self, img, pos, text, canskip, t, hscale, vscale, num, px, py, intv, snd, vol, img_upper)
    intv = intv or 1
    hscale = hscale or 1
    vscale = vscale or hscale or 1
    num = num or 1
    ...
    for i = 1, l, intv do
        ...
        lib.sp_sentence(self, img, pos, text_slice, canskip, 1, hscale, vscale, num, px, py, snd, vol, img_upper, text,
            newtext)
        PlaySound(snd, vol, 0, true)
    end
    lib.sp_sentence(self, img, pos, newtext, canskip, t - dt, hscale, vscale, num, px, py, snd, vol, img_upper, text,
        newtext)
end

--v1.01a的multi_sentence，因效率较低、稳定性较差而移除
--若想实现在第n句对话后插入函数，可用以下方法实现：
--lib.sp_multi_sentence(...,1,n)
--func(...)
--lib.sp_multi_sentence(...,n+1,#text)

--[=[
---多句对话，适用于对话文字量大、需批量管理的场所（指私坑）
---传intv则使用sentence_ex，否则使用普通sentence
---传参格式化的规则：
...
---传入parameter的格式：
---{{param1 of func1,param2 of func1,...},{param1 of func2,...},...}
---即每个函数的参数需分别放在一个列表中，若无参数则请传入空表{}
---multi_sentence不会对parameter进行格式检查，也不会对其进行自动补齐，请自行注意格式
---该功能不稳定，请谨慎使用
...
---@param intv table|number @每帧显示字符数（按utf8长度计算）
---@param snd table|string @对话语音
---@param vol table|number @对话语音音量
---@param func table|function @需要在某句对话前/后调用的函数
---@param funcpos table|string @函数调用位置，可选'before' | 'after'
---@param parameter any @函数所需参数，传入格式见上
function lib:multi_sentence2(self, img, pos, text, canskip, t, hscale, vscale, intv, snd, vol, func, funcpos, parameter)
    local IsEX
    if intv then IsEX = true end
    if type(text) ~= 'table' then text = { text } end
    local default_param = { 'img_void', 'right', '', true, 60, 1, 1, 3, 'plst00', 0.35, function() end, 'before' } --默认参数表
    local param = { img, pos, text, canskip, t, hscale, vscale, intv, snd, vol, func, funcpos } --传入参数表
    --格式化参数，确保每个参数都是长度与text相同的表
    for i = 1, 12 do
        if type(param[i]) == 'table' then
            for k, v in ipairs(param[i]) do
                if v == 'default' then param[i][k] = default_param[i] end
            end
        elseif param[i] == 'default' then
            param[i] = default_param[i]
        end
        if type(param[i]) ~= 'table' then
            if type(param[i]) == 'nil' then
                param[i] = {}
                for j = 1, #text do
                    table.insert(param[i], default_param[i])
                end
            else
                local temp_param = param[i]
                param[i] = {}
                for j = 1, #text do
                    table.insert(param[i], temp_param)
                end
            end
        elseif #param[i] < #text then
            local temp_param = param[i][#param[i]]
            for j = 1, #text - #param[i] do
                table.insert(param[i], temp_param)
            end
        end
    end
    --批量输出对话
    for i = 1, #text do
        if func and func[i] and parameter and parameter[i] and funcpos[i] == 'before' then
            func[i](unpack(parameter[i]))
        end
        if IsEX then
            lib:sentence_ex(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i], param[7][i], param[8][i], param[9][i], param[10][i])
        else
            lib:sentence(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i], param[7][i], param[9][i], param[10][i])
        end
        if func and func[i] and parameter and parameter[i] and funcpos[i] == 'after' then
            func[i](unpack(parameter[i]))
        end
    end
end
...
function lib.sp_multi_sentence(self, img, pos, text, canskip, t, hscale, vscale, num, px, py, intv, snd, vol, img_upper,
                               start_pos, end_pos)
    local IsEX
    if intv then IsEX = true end
    if type(text) ~= 'table' then text = { text } end
    ...
    for i = start_pos, end_pos do
        if IsEX then
            lib.sp_sentence_ex(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i],
                param[7][i], param[8][i], param[9][i], param[10][i], param[11][i], param[12][i], param[13][i],
                param[14][i])
        else
            lib.sp_sentence(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i],
...
---@param start_pos number @对话起始位置
---@param end_pos number @对话结束位置
function lib.sp_multi_sentence_ex(self, img, pos, text, canskip, t, hscale, vscale, num, px, py, intv, snd, vol,
...
---boss对话显示器
lib.boss_dialog_displayer = Class(object)
---@param p_dialog boolean 是否将player.dialog设置为true
---@param dialogbox_blend string @对话框渲染模式
---@param dialogbox_co1 lstg.Color @对话框颜色1（左）
---@param dialogbox_co2 lstg.Color @对话框颜色2（右）
...
---@param text_co2 lstg.Color @对话字体颜色2（右）
---@param dialogTTF string @对话字体（TTF）
function lib.boss_dialog_displayer:init(p_dialog, dialogbox_blend, dialogbox_co1, dialogbox_co2, text_co1, text_co2,
...
function lib.boss_dialog_displayer:frame()
    task.Do(self)
    if self.t > 0 then
        self.t = self.t - 1
    end
...
function lib.boss_dialog_displayer:render()
end

function lib.boss_dialog_displayer:del()
...
---@param px number @方位图像x坐标 --立绘图像坐标，注意这里是ui系坐标（太nt了，下次我绝对要改掉
...
function lib.boss_sentence(self, img, pos, text, canskip, t, hscale, vscale, tpic, num, px, py, tx, ty, tn, stay, snd,
    ...
    local balloon = New(lib.balloon, master, tx, ty, pos, 1, tpic, newtext, tn, master.blend, balloon_co, text_co, master.text_scale)
...
---@param snd string @对话语音
---@param vol number @对话语音音量
function lib.boss_sentence_ex(self, img, pos, text, canskip, t, hscale, vscale, tpic, num, px, py, tx, ty, stay, intv,
                              snd, vol)
    intv = intv or 1
    hscale = hscale or 1
    vscale = vscale or hscale or 1
    ...
    for i = 1, l, intv do
        ...
        local text_slice = sp.string(newtext):Sub(1, i)
        lib.boss_sentence(self, img, pos, text_slice, canskip, 1, hscale, vscale, tpic, num, px, py, tx, ty, tn, stay,
            snd, vol, text,
            newtext, true)
        PlaySound(snd, vol, 0, true)
    end
    lib.boss_sentence(self, img, pos, newtext, canskip, t - dt, hscale, vscale, tpic, num, px, py, tx, ty, tn, stay, snd,
        vol, text,
        newtext)
end
...
---@param start_pos number @对话起始位置
---@param end_pos number @对话结束位置
function lib.boss_multi_sentence(self, img, pos, text, canskip, t, hscale, vscale, tpic, num, px, py, tx, ty, tn, stay,
    ...
    for i = start_pos, end_pos do
        if IsEX then
            lib.boss_sentence_ex(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i],
                param[7][i], param[8][i], param[9][i], param[10][i], param[11][i], param[12][i], param[13][i],
                param[15][i], param[16][i], param[17][i], param[18][i])
        else
            lib.boss_sentence(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i],
                param[7][i], param[8][i], param[9][i], param[10][i], param[11][i], param[12][i], param[13][i],
...
---@param start_pos number @对话起始位置
---@param end_pos number @对话结束位置
function lib.boss_multi_sentence_ex(self, img, pos, text, canskip, t, hscale, vscale, tpic, num, px, py, tx, ty, stay,
                                    intv, snd, vol, start_pos, end_pos)
    if type(text) ~= 'table' then text = { text } end
    local posn
    if pos == 'left' or pos == 1 then
...
---float对话显示器
lib.float_dialog_displayer = Class(object)
---@param p_dialog boolean 是否将player.dialog设置为true
---@param dialogbox_blend string @对话框渲染模式
---@param dialogbox_co1 lstg.Color @对话框颜色
---@param text_co lstg.Color @对话字体颜色
---@param dialogTTF string @对话字体（TTF）
function lib.float_dialog_displayer:init(p_dialog, dialogbox_blend, dialogbox_co, text_co, text_scale, dialogTTF)
    self.type = 'float'
    self.layer = LAYER_TOP + 10
    self.char = {}
    self.char[1] = {}
...
function lib.float_dialog_displayer:frame()
    task.Do(self)
    if self.t > 0 then
        self.t = self.t - 1
    end
...
function lib.float_dialog_displayer:render()
end

function lib.float_dialog_displayer:del()
...
---@param fulltext string @已处理文字效果但未截取的text（由sentence_ex使用）
---@param balloon_ex boolean @sentence_ex的标志，会影响对balloon.n的处理（由sentence_ex使用）
function lib.float_sentence(self, text, canskip, t, tpic, tx, ty, tn, snd, vol, rawtext, fulltext, balloon_ex)
    tx = tx or -10
    ty = ty or -25
    tpic, tn = tpic or 1, tn or 1
    snd = snd or 'plst00'
    ...
    local balloon = New(lib.balloon_float, master, x, y, 1, tpic, newtext, tn, master.blend, master.balloon_co, master.text_co, master.text_scale)
    table.insert(master.balloon, balloon)
    --lastsentence = balloon
    t = t or (60 + #text * 5)
    for _ = 1, t do
...
---@param snd string @对话语音
---@param vol number @对话语音音量
function lib.float_sentence_ex(self, text, canskip, t, tpic, tx, ty, intv, snd, vol)
    intv = intv or 1
    tx = tx or -10
    ty = ty or -25
    tpic = tpic or 1
    ...
    for i = 1, l, intv do
        ...
        local text_slice = sp.string(newtext):Sub(1, i)
        lib.float_sentence(self, text_slice, canskip, 1, tpic, tx, ty, tn, snd, vol, text,
            newtext, true)
        PlaySound(snd, vol, 0, true)
    end
    lib.float_sentence(self, newtext, canskip, t - dt, tpic, tx, ty, tn, snd,
        vol, text, newtext)
end
...
---@param start_pos number @对话起始位置
---@param end_pos number @对话结束位置
function lib.float_multi_sentence(self, text, canskip, t, tpic, tx, ty, tn, intv, snd, vol, start_pos, end_pos)
    local IsEX
    if intv then IsEX = true end
    if type(text) ~= 'table' then text = { text } end
    ...
    for i = start_pos, end_pos do
        if IsEX then
            lib.float_sentence_ex(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i],
                param[8][i], param[9][i], param[10][i])
        else
            lib.float_sentence(self, param[1][i], param[2][i], param[3][i], param[4][i], param[5][i], param[6][i],
...
---@param start_pos number @对话起始位置
---@param end_pos number @对话结束位置
function lib.float_multi_sentence_ex(self, text, canskip, t, tpic, tx, ty, intv, snd, vol, start_pos, end_pos)
    if type(text) ~= 'table' then text = { text } end
...
function lib.TTFDrawer:render(font, x1, x2, y1, y2, cw, ch, dx, dy, scale, color, align, _align)
    ...
    if flag then
        for _, v in ipairs(flag) do
            if v == 'pef' and lstg.tmpvar.TextEffect_RT_Created then
    ...
    for i = 1, #s do
        ...
        if flag then
            for k, v in ipairs(flag) do
                if i == st[k] then
                    if v == 'fon' then font = param[k] end
                    if v == 'sca' then
                        scale = lib.StrToNum(param[k])
                        if self.balloon then self.balloon.balloon_scale = max(self.b_scale,
                                default_scale + (scale - default_scale) / 2) end
                    end
                    if v == 'col' then color = Color(lib.StrToNum(param[k])) end
                    if v == 'Col' then
        ...
        --[[if flag then
            for k, v in ipairs(flag) do
                if v == 'pef' and i >= st[k] and (i >= e[k] or i == #s) then
                    PopRenderTarget()
...
function balloon:frame()
    ...
    self.scale = t / 10]]--为了sentence_ex调用，这里放弃了气泡逐渐变大的效果
...
---float系对话气泡
---@class lib.balloon_float
---@return lib.balloon_float
lib.balloon_float = Class(object)
local balloon_float = lib.balloon_float
function balloon_float:init(master, x, y, vpos, pic, text, n, balloon_blend, balloon_co, text_co, text_scale)
    self.layer = LAYER_TOP + 233 --无上至尊（啥
    self.master = master
    self.x, self.y = x, y
    self.bound = false
    ...
    self.l = max(10, self.l) --由于某些奇妙问题，float气泡在长度小于10时就会出问题
    self.tx = self.l * 16
    self.n = n or 1
...
function balloon_float:render()
    ...
    y = y - 25 * (self.balloon_scale - 1) --暴力调参，但有效
    x = x + 14 * self.balloon_scale --梅开二度，这次是为了float气泡的偏差
...
---@param text_co2 lstg.Color @对话字体颜色2（右）
---@param dialogTTF string @对话字体（TTF）
function lib.sp_SetDisplayer(self, p_dialog, dialogbox, dialogbox_hscale, dialogbox_vscale, dialogbox_yu, text_yu,
    ...
    self.dialog_displayer = New(lib.sp_dialog_displayer, p_dialog, dialogbox, dialogbox_hscale, dialogbox_vscale,
...
---@param text_co2 lstg.Color @对话字体颜色2（右）
---@param dialogTTF string @对话字体（TTF）
function lib.boss_SetDisplayer(self, p_dialog, dialogbox_blend, dialogbox_co1, dialogbox_co2, text_co1, text_co2,
                               text_scale, dialogTTF)
    if IsValid(self.dialog_displayer) then Del(self.dialog_displayer) end
    p_dialog = p_dialog or true
    dialogbox_blend = dialogbox_blend or ''
    ...
    self.dialog_displayer = New(lib.boss_dialog_displayer, p_dialog, dialogbox_blend, dialogbox_co1, dialogbox_co2,
...
---@param text_co lstg.Color @对话字体颜色
---@param dialogTTF string @对话字体（TTF）
function lib.float_SetDisplayer(self, p_dialog, dialogbox_blend, dialogbox_co, text_co, text_scale, dialogTTF)
    if IsValid(self.dialog_displayer) then Del(self.dialog_displayer) end
    p_dialog = p_dialog or true
    dialogbox_blend = dialogbox_blend or ''
    ...
    text_scale = text_scale or 1
    dialogTTF = dialogTTF or 'balloon_font'
    self.dialog_displayer = New(lib.float_dialog_displayer, p_dialog, dialogbox_blend, dialogbox_co, text_co, text_scale, dialogTTF)
end
...
function lib.StrToNum(str)
    if str == 'ranA' then
        return ran:Float(0, 360)
    elseif str == 'ranB' then
        return ran:Float(0, 1)
    elseif str == 'ranC' then
        return ran:Sign()
    end
    return tonumber(str)
end
...
--[=[
---将string类型的数字转换为number
---利用了lua在试图使string与number运算时会尝试将string转换为number的特性
---安全性不佳，所以需要预防报错
---@param str string @要转换的数字（string）
---@return number @转换后的数字（number）
function lib.StrToNum(str)
    ...
    --原本设计的语法，但由于lib.UnpackParamList使用逗号数量来判断参数数量，这样写有问题
    --只能希望上面几个常量够用了
    ...
    --return string.match(str, '%d+') end --利用了match%d会自动转换的特性
...
---@return number @table的最大值
function lib.TableMax(t)
    ...
    if n > -1145141919810 then return n end
end
...
--本函数作用为接受文本，读取文字效果信息后删去文字效果，并将文字效果信息传递给具体实现处
--对本函数作用一个可能并不太好的比喻：
--类似于细胞的内质网接受半成品肽链后对其进行裁剪与加工，之后将蛋白质传递至高尔基体
--学生物学的（
...
function lib.GetTextEffect(text)
    if string.match(text, '<%w+ .+ >') then
        local flag, param = string.match(text, '<(%w+) (.+) >') --获取文字效果类型和参数
        if string.match(flag, '%u') then --检测到flag大写时自动解包参数列表
            param = lib.UnpackParamList(param)
        end
        local l = string.match(text, '<%w+ .+ >') --左侧部分
        local r = string.match(text, '<end>') --右侧部分
        local sl, el, sr, er = 0, 0, sp.string(text):GetCharCount(), sp.string(text):GetCharCount() --左侧与右侧始末位置
        sl, el = string.find(lib.SpstringTrans(text), '<%w+ .+ >')
        local temp = string.gsub(text, '<%w+ .+ >', '')
        if r then
            sr, er = string.find(lib.SpstringTrans(temp), '<end>')
            text = string.gsub(temp, r, '')
        else
            text = temp
        end
        local len = sp.string(l):GetCharCount() --左侧部分长度
        flag = { flag }
        param = { param }
        el = { el + 1 - len } --兼容多文字效果
        if r then
...
---获取文本中文字效果(多效果版)
---使用时务必确保<end>数量与文字效果数量相等且每个<end>均含有顺序编号
---制作未完成，match范围超过预期的问题仍未解决
---问题如下：例如传入text为'<col1 0xFFFFFFFF ><Col2 {255,255,255,255} >text<end1><end2>'，则match时会匹配到'<col1 0xFFFFFFFF ><Col2 {255,255,255,255} >'而不是只匹配一个
---这需要一个好的语法结构来解决，然而目前我想不到
---由于本函数兼容单文字效果，暂时这么放在这问题也不大
---@param text string @传入的文本
---@return string,table,table,table,table @返回删去文字效果后的文本，文字效果类型，参数，文字效果起始与终止位置
---需要注意这里返回的参数是string类型，位置是以字符数计算的
function lib.MultiGetTextEffect(text)
    if string.match(text, '<%w+ .+ >') then
        if not string.match(text, '<%w+%d .+ >') then return lib.GetTextEffect(text) end --如果只有一个文字效果则换回普通GetTextEffect
        local paramnum = 0 --文字效果数量
        local defaultflag = {}
        local defaultend = {}
        for t in string.gmatch(text, '<%w+%d .+ >') do
            table.insert(defaultflag, string.match(t, '<(%w+) .+ %d>'))
        end
        for t in string.gmatch(text, '<end%d>') do
            table.insert(defaultend, t)
        end
        if paramnum == 1 then return lib.GetTextEffect(text) end    --同上
        local flag, param, sl, el, sr, er, len = {}, {}, {}, {}, {} --含义同普通GetTextEffect
        local num = 0
        for _ = 1, paramnum do
            num = num + 1
            local l = string.match(text, '<%w+' .. paramnum .. ' .+ >')
            local f, p = string.match(l, '<(%w+)' .. paramnum .. ' (.+) >') --获取文字效果类型和参数（按编号顺序）
            table.insert(flag, f)
            if string.match(p, '%u') then
                table.insert(param, lib.UnpackParamList(p))
            else
                table.insert(param, p)
            end
            local tempsl, tempel = string.find(lib.SpstringTrans(text), l)
            --table.insert(sl, tempsl)
            table.insert(el, tempel)
            local tempsr, temper = string.find(lib.SpstringTrans(text), '<end' .. paramnum .. '>')
            table.insert(sr, tempsr)
            --table.insert(er, temper)
            table.insert(len, sp.string(l):GetCharCount())
        end
        for i, v in ipairs(el) do
            for j, k in ipairs(defaultflag) do
                if flag[i] == k then --检索左半部分实际位置
                    el[i] = v - lib.TableSum(len, 1, j - 1) + 1 --计算删去文字效果后起始位置
                end
            end
        end
        for i, v in ipairs(sr) do
            for j, k in ipairs(defaultend) do
                if '<end' .. i .. '>' == k then --检索右半部分实际位置
                    sr[i] = v - (j - 1) * 6 - 1 --计算删去文字效果后终止位置
                end
            end
        end
        text = string.gsub(text, '<%w+ .+ >', '') --删去文字效果
        text = string.gsub(text, '<end%d>', '')
        return text, flag, param, el, sr
    end
    return text
end
