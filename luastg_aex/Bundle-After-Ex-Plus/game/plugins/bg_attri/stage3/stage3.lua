...
function stage3_bg:init()
    ...
    for i=1,self.tree1num do
        self.tree1dx[i] = math.random() - 0.5
        while math.abs(self.tree1dx[i]) < 0.1 do
            self.tree1dx[i] = math.random() - 0.5
        end
    end
...
function stage3_bg:frame()
    ...
    while self.tree1dx[self.tree1num] == nil do
        ...
        while math.abs(dx) < 0.1 do
            dx = math.random() - 0.5
        end
        table.insert(self.tree1dx,math.random() - 0.5)
    end
...
local function draw_tree1(z,x,num,itv)
    for j=num,0,-1 do
        local dx = x[j+1]
        local dz = j*itv-z
        Render4V('wood', 0.02+dx,-0.5,dz,
                            -0.02+dx,-0.5,dz,
                            -0.02+dx,-1,dz,
                            0.02+dx,-1,dz)
        Render4V('tree1', 0.25+dx,-0.25,dz,
                            -0.25+dx,-0.25,dz,
                            -0.25+dx,-0.75,dz,
                            0.25+dx,-0.75,dz)
        Render4V('tree1', 0.175+dx,0,dz,
                            -0.175+dx,0,dz,
                            -0.175+dx,-0.5,dz,
                            0.175+dx,-0.5,dz)
    end
...
function stage3_bg:render() 
    ...
    if statu == 1 then
        ...
        draw_tree1(dz,self.tree1dx,self.tree1num-1,self.tree1itv)
    end
    if statu == 2 then
        local dy = -self.y
        for j=0,4 do
